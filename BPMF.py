...
class DataSet:
    ...
    def _getDataset_as_list(self):
        ...
        print('Dataset Statistics: Interaction = %d, User = %d, Item = %d, Sparsity = %.4f' % \
              (data.shape[0], maxu - 1, maxi - 1, data.shape[0] / (maxu * maxi)))
        trainset = data.values.tolist()
        # testset
        filePath = r"./testset_100k.csv"
        data = pd.read_csv(filePath, sep=',', header=None, names=['user', 'item', 'rating'], \
                           usecols=[0, 1, 2], dtype={0: np.int32, 1: np.int32, 2: np.float})
...
def getMAE_RMSE(testSet, predict):
    ...
    for u, i, r in testSet:
        error = math.fabs(r - predict[int(u),int(i)])
        maeError.append(error)
        rmseError.append(error * error)
...
def getF_value(testSet,predict):
    ...
    for u, i, r in testSet:
        actual_num += 1.0
        if round(r) == round(predict[int(u),int(i)]):
            perfect_num += 1.0
...
if __name__ == "__main__":
    ...
    for K in [10]:
        ...
        R_pred = BPMF(R, R_test, U_in, V_in, T=20, D=K, initial_cutoff=0, lowest_rating=0, highest_rating=ds.maxr)
